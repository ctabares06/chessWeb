...
const Game: React.FC = () => {
	...
	const {
		...
		name: whiteName,
		color: whiteColor,
	} = useBearStore((state) => state.game[Sides.white]);
	const {
		graveyard: blackGraveyard,
		name: blackName,
		color: blackColor,
	} = useBearStore((state) => state.game[Sides.black]);
	const playerTurn = useBearStore(
		(state) => state.game[state.game.turn].name
	);
